	...
	export default {
		...
		product_data(new_value) {
			...
			this.image_preview = this.product.file.route
 			// this.image_preview = this.product.file ? this.product.file.route : '/storage/images/products/default.png';
		}
	},
		computed: {
			schema() {
...
async saveProduct() {
    try {
        ...
        // Crear el objeto FormData solo si se ha seleccionado un nuevo archivo
        const product = this.file ? this.createFormData(this.product) : this.product;
        ...
        // Maneja el manejo de errores según tu lógica
    }
},
